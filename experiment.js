...
function fitGjtSentenceToOneLine() {
  const sentence = document.querySelector(".gjt-sentence");
  if (!sentence || window.innerWidth > 600) return;

  // Start at a readable size, then reduce only as much as needed.
  let fontSize = 24;
  const minimumFontSize = 15;
  sentence.style.fontSize = `${fontSize}px`;
  sentence.style.whiteSpace = "nowrap";

  while (sentence.scrollWidth > sentence.clientWidth && fontSize > minimumFontSize) {
    fontSize -= 0.5;
    sentence.style.fontSize = `${fontSize}px`;
  }

  // Extremely narrow screens may still require wrapping rather than clipping.
  if (sentence.scrollWidth > sentence.clientWidth) {
    sentence.style.whiteSpace = "normal";
    sentence.style.fontSize = `${minimumFontSize}px`;
  }
}
...
function makeGjtTrial(isPractice = false) {
  return {
    ...
    on_load: function() {
      requestAnimationFrame(fitGjtSentenceToOneLine);
    },
    data: function() {
      if (isPractice) {
        return {
