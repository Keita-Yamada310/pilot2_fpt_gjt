...
- FPT練習項目に `acceptable_alternative` の空値を明示しました。
- FPT本番の開始時刻を「課題1を始める」ボタン押下後に設定するよう修正しました。
- JavaScriptエラー発生時に、空白画面ではなくエラー案内を表示します。
...
- 左ボタン：正しい
- 右ボタン：間違い
- 練習2問では各回答後に「正解／不正解／時間切れ」と正しい判断を表示します。
- 本課題ではフィードバックを表示しません。

## DataPipe保存エラーについて

`config.js` の `DATAPIPE_EXPERIMENT_ID` が空欄の場合、OSFには保存されません。
本番実施前にDataPipeで発行されたExperiment IDを入力してください。
空欄のまま開始すると、開始時に担当者向け警告が表示されます。
